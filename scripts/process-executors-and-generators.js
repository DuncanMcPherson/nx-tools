...
function processExecutorAndGenerators() {
	...
	if (fs.existsSync('./executors.json')) {
		...
		Object.keys(executorsJson.executors).forEach((key) => {
			...
			if (!implementationPathParts.includes('dist')) {
				const newArray = [
			...
			if (!schemaParts.includes('dist')) {
				...
				console.log(`Schema path for executor ${key} was updated to: ${newSchemaPath.join('/')}`);
			}
		});
	...
	if (fs.existsSync('./generators.json')) {
		...
		Object.keys(generatorsJson.generators).forEach((key) => {
			...
			if (!factoryPathParts.includes('dist')) {
				const newArray = [
			...
			if (!schemaParts.includes('dist')) {
				...
				console.log(`Schema path for generator ${key} was updated to: ${newSchemaPath.join('/')}`);
			}
		});
