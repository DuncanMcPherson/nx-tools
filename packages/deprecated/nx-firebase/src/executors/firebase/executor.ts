...
function normalizeOptions(
	options?: FirebaseExecutorSchema
): INormalizedExecutorSchemaOptions {
	...
	opts.readyWhen ??=
		'readyWhen' in options
			? options.readyWhen
			: 'It is now safe to connect';
...
function stringifyCommandAndOpts(
	command: string,
	options: INormalizedExecutorSchemaOptions
): string {
	let opts = '';
