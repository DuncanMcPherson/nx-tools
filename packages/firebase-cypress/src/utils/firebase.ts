...
export function detectFirebase(context: ExecutorContext): {
  ...
  const { hasEmulators, portNumber } = getPortForFirebaseEmulator(
    join(context.root, dependency.root, 'firebase.json')
  );

  return hasEmulators
    ? { isPresent: true, portNumber, projectRoot: dependency.root }
    : { isPresent: false };
}
...
async function waitForServer(port: number): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Emulators failed to start in ${timeoutDuration}ms. Cancelling the operation.`
        )
      );
    }, timeoutDuration);
