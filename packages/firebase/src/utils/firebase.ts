...
function getFirstEmulatorPort(context: ExecutorContext, only?: string): number {
  ...
  if (!existsSync('firebase.json')) {
    ...
    if (!existsSync(joinPathFragments(projectRoot, 'nx-firebase.json'))) {
      const message = chalk.bgKeyword('orange')(
        "The process has detected that you are running a version of this plugin that does not have the nx-firebase.json file present. This will not be supported in v2. Please run 'nx migrate @nxextensions/firebase@v1.3.0' to fix this issue"
      );
