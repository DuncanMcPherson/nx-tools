...
describe('Firebase Executor', () => {
	...
	it('should throw error when parallel is false and readyWhen is passed', async () => {
		...
		try {
			...
			fail(`An error should have been thrown but received success: ${output.success}`);
		} catch (e) {
			expect(e).toBeInstanceOf(Error);
			expect(e.message).toEqual('The "--parallel" flag must be set to true in order to use "--readyWhen"');
		}
	});
