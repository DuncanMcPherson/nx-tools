import {
  ...
  CreateNodesV2, detectPackageManager,
  getPackageManagerCommand, joinPathFragments, normalizePath, NxJsonConfiguration,
...
async function createNodesInternal(configFile: string, options: object, context: CreateNodesContext, targetsCache: Record<string, PluginTargets>) {
...
async function buildCypressTargets(
  ...
  const webServerCommands: Record<string, string> = pluginPresetOptions.webServerCommands;
  ...
  if ('e2e' in cypressConfig) {
    ...
    if (ciWebServerCommand) {
      const specPatterns: string[] = Array.isArray(cypressConfig.e2e.specPattern)
      ...
      const excludeSpecPatterns: string[] =
        !cypressConfig.e2e.excludeSpecPattern
          ...
          : Array.isArray(cypressConfig.e2e.excludeSpecPattern)
            ? cypressConfig.e2e.excludeSpecPattern.map((p: string) => join(projectRoot, p))
